...
class DatabaseQuery
{
    ...
    {
        return "SELECT mv.idconcepto, mv.idperiodo, mv.importetotal FROM [nom10007] mv, [nom10002] pr WHERE mv.idempleado = {$worker_id} AND pr.idperiodo = mv.idperiodo AND pr.fechainicio BETWEEN {$date_begin} AND {$date_end} AND pr.ejercicio = {$exercise} AND pr.idtipoperiodo = {$period_type} AND mv.importetotal != 0 ORDER BY mv.idperiodo";
    }
